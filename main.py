...
# Inicializar el cliente oficial estable global de Google GenAI (Ruta v1 estándar)
...
async def generar_imagen(prompt: str = Form(...), ratio: str = Form("1:1")):
    try:
        # Validar formatos compatibles oficiales de la SDK para Imagen
        formato_valido = ratio if ratio in ["1:1", "3:4", "4:3", "16:9", "9:16"] else "1:1"

        # Cambiamos al modelo activo de acceso general de Google GenAI
...
async def editar_imagen(
    ...
    try:
        ...
        # Ajuste al modelo homólogo activo para tareas de edición de imagen
